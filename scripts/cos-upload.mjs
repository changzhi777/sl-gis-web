/**
 *  * cos-upload.mjs — 零依赖上传 dist/ 到腾讯云 COS（CNB CI 用）
  * 自实现 COS XML API 签名，无需 SDK
                 ...
                 const httpString = method.toLowerCase() + '\n' + pathname.toLowerCase() + '\n\nhost=' + host.toLowerCase() + '\n';
                   const stringToSign = 'sha1\n' + keyTime + '\n' + sha1hex(httpString) + '\n';
                     const sig = hmac(signKey, stringToSign);
                       return 'q-sign-algorithm=sha1&q-ak=' + SECRET_ID + '&q-sign-time=' + keyTime + '&q-key-time=' + keyTime + '&q-header-list=host&q-url-param-list=&q-signature=' + sig;
                       }
                                                                           ...
                                                                           'Content-Type': MIME[ext] || 'application/octet-stream',
                                                                                 'Cache-Control': 'public, ' + cache,
                                                                                         ...
                                                                                         body: readFileSync(f),
                                                                                             ...
                                                                                             if (res.ok) { ok++; console.log('  OK', relative(DIST, f)); }
                                                                                               else { fail++; console.error('  FAIL', relative(DIST, f), res.status, (await res.text()).slice(0, 120)); }
                                                                                               }
