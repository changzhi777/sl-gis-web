 ...
 * 全部为静态常量，无随机数，多次刷新画面一致；2 期切后端接口时仅替换本文件
 */
...
/* ============ 信息公告（四类标签 × 三渠道） ============ */
export type NoticeType = '停水公告' | '限时供水' | '水质检测' | '恢复供水';
export type NoticeChannel = '公众号' | '小程序' | '服务热线';
...
export const NOTICE_TYPE_META: Record<NoticeType, { label: string; color: string }> = {
  停水公告: { label: '停水', color: 'var(--status-alarm)' },
...
export const CHANNEL_META: Record<NoticeChannel, { icon: string; label: string; color: string }> = {
  ...
  服务热线: { icon: '话', label: '热线', color: 'var(--steppe-amber)' },
};

export const notices: NoticeItem[] = [
  { id: 'N-0910-01', type: '停水公告', title: '朱日和镇区主管网抢修停水（07:00–13:00）', channel: '小程序', publishAt: '09-10 05:30', status: '已发布' },
  ...
  { id: 'N-0909-04', type: '限时供水', title: '查干淖尔嘎查限时供水（06:00–09:00 / 17:00–20:00）', channel: '服务热线', publishAt: '09-09 07:45', status: '已发布' },
  { id: 'N-0908-05', type: '恢复供水', title: '乌兰陶勒盖嘎查管网冲洗消毒完成，恢复供水', channel: '小程序', publishAt: '09-08 15:10', status: '已过期' },
  { id: 'N-0908-06', type: '停水公告', title: '赛罕塔拉嘎查泵房年度检修停水 8 小时', channel: '服务热线', publishAt: '09-08 08:00', status: '已过期' },
  ...
  { id: 'N-0906-09', type: '停水公告', title: '都希乌苏嘎查水源井清洗消毒停水 6 小时', channel: '服务热线', publishAt: '09-06 07:00', status: '已过期' },
  { id: 'N-0906-10', type: '恢复供水', title: '哈日阿图嘎查新水源井启用，供水全面恢复', channel: '公众号', publishAt: '09-06 12:40', status: '已过期' },
];

/* ============ 报修 / 投诉工单池 ============ */
export type TicketStage = '待受理' | '处理中' | '待回访' | '已办结';
...
/** 进度管道四阶段（顺序即流转方向） */
export const STAGE_ORDER: TicketStage[] = ['待受理', '处理中', '待回访', '已办结'];

/** 阶段着色：待受理暖 / 处理中青 / 待回访紫 / 已办结绿 */
...
export const tickets: TicketItem[] = [
  ...
  { id: 'GT-0909-002', type: '投诉', village: '乌日根塔拉镇查干淖尔嘎查', summary: '夜间停水频繁，请求错峰公示', stage: '待回访', handler: '娜仁高娃', elapsed: '20h' },
  { id: 'GX-0909-005', type: '报修', village: '哈日阿图嘎查', summary: '管道排气阀失效，气堵不出水', stage: '待回访', handler: '朝鲁', elapsed: '14h' },
